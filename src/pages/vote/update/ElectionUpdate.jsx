...
function ElectionUpdate() {
  ...
  useEffect(() => {
    fetch(`http://localhost:8080/election/getelection/${id}`)
      .then((res) => res.json())
      .then((result) => {
        setList(result);
        setTitle(result.electionName);
        setDate(result.electionDate);
        setDesc(result.electionDescription);
      });
  }, [isAdding]);
  ...
  const SaveElection = (e) => {
    ...
    fetch(`http://localhost:8080/election/update/${id}`, {
      method: "PUT",
      headers: {
